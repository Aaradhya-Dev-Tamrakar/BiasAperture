...
The architecture follows Object-Oriented Software Engineering practice under \gls{solid} guidelines, applied concretely rather than generically. The single-responsibility principle is realised directly by the five-module split in \cref{sec:modules}: each module owns exactly one concern, and no module reaches into another's internal state. The open-closed principle is realised by the Strategy pattern around the fairness-metrics back end (\cref{sec:patterns}): a future fifth disparity metric, or a replacement fairness library, is added by implementing a new strategy without modifying the computation engine's internals. The dependency-inversion principle is realised by having the engine and report generator depend only on the abstract \texttt{FairnessBackend} and \texttt{ModelInterface} interfaces, never on AIF360, Fairlearn, or a specific file format directly, so that a cut or substitution at either boundary (\cref{sec:cutlist}) changes an implementation, not a contract. Finally, the shared schema described in \cref{sec:schedule} is an application of interface-first, design-by-contract practice: the field set two independently developed streams must both honour is fixed before either stream begins substantive work.

\section{Architectural Modules}
\label{sec:modules}

BiasAperture is organised into five cooperating modules, summarised in \cref{tab:modules} and detailed below. Each module exposes a narrow, well-defined interface, a dataset object, a prediction table, a metrics dictionary, or a report object, so that it can be developed, unit-tested, and extended independently of the others (NFR-005).
...
The five modules cooperate through a fixed five-step workflow: (1)~\emph{Configuration}, the auditor selects a benchmark dataset or supplies a custom one, specifies the target model or a predictions file, and selects the metrics of interest; (2)~\emph{Ingestion}, the data ingestion module loads and validates the dataset; (3)~\emph{Inference}, the model interface module obtains predictions; (4)~\emph{Computation}, the fairness engine and explainability layer compute metrics, run significance tests, and attribute flagged disparities; and (5)~\emph{Reporting}, the report generation module produces the standardised, regulator-traceable fairness report.
...
\begin{table}[htbp]
    ...
    \label{tab:patterns}
    \begin{tabular}{p{2.2cm}p{3cm}p{2.8cm}p{5.5cm}}
        \toprule
        \textbf{Pattern} & \textbf{Module} & \textbf{Structure} & \textbf{Rationale} \\
        \midrule
        Adapter & Model Interface & \texttt{ModelInterface} $\to$ \texttt{DirectInferenceAdapter} / \texttt{PredictionsFileAdapter} & Cutting direct inference (\cref{sec:cutlist}, item 4) means simply not instantiating one adapter; downstream code is unchanged \\
        Strategy & Fairness Metrics Engine & \texttt{FairnessBackend} $\to$ \texttt{AIF360Backend} / \texttt{FairlearnBackend} & Cutting AIF360 (\cref{sec:cutlist}, item 5) is a backend swap, not a rewrite; the four metrics are interchangeable strategies \\
        Builder & Data Ingestion & \texttt{TestMatrixBuilder} & Matches the multi-step curation (acquisition, integrity check, schema alignment) the module performs \\
        Factory & Report Generation & \texttt{ReportFactory} $\to$ \texttt{HTMLReportBuilder} & Keeps the \gls{html} report path untouched regardless of which export formats are in scope \\
        Template Method & Report Generation & \texttt{AuditReport} base class & Fixes the Model Cards / Datasheets section skeleton while allowing subclasses to fill format-specific hooks \\
        Facade & Orchestration & \texttt{AuditOrchestrator} & Hides the ingestion~$\to$~interface~$\to$~engine~$\to$~report sequence behind one call \\
        \bottomrule
    \end{tabular}
\end{table}
