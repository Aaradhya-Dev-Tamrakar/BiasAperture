...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.2cm}p{2.8cm}p{7.3cm}}
        \toprule
        \textbf{Field} & \textbf{Type} & \textbf{Description} \\
        \midrule
        \texttt{image\_id} & string & Unique identifier for the source image \\
        \texttt{race} & categorical (7 levels) & FairFace race / ethnicity category \\
        \texttt{gender} & categorical (binary) & Perceived gender label \\
        \texttt{age} & categorical (9 bins) & FairFace age-group bin \\
        \texttt{true\_label} & categorical & Ground-truth demographic attribute value \\
        \texttt{predicted\_label} & categorical & Value returned by the Model Interface module \\
        \texttt{protected\_attribute} & categorical & Subgroup field currently selected for disparity analysis \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.5cm}p{2.5cm}p{7.2cm}}
        \toprule
        \textbf{Field} & \textbf{Type} & \textbf{Description} \\
        \midrule
        \texttt{metric\_name} & string & One of \gls{dpd}, \gls{eod}, \gls{eop}, \gls{dir} \\
        \texttt{subgroup\_n} & integer & Subgroup sample size \\
        \texttt{point\_estimate} & float & Computed metric value \\
        \texttt{ci\_lower}, \texttt{ci\_upper} & float pair & 95\% bootstrap confidence interval bounds ($\geq$1{,}000 resamples) \\
        \texttt{p\_value} & float & Result of the chi-squared significance test \\
        \texttt{insufficient\_sample\_flag} & boolean & True when subgroup \gls{symb:n} $< 30$ (NFR-003) \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{p{3.3cm}p{1.6cm}p{1.6cm}p{6.5cm}}
        \toprule
        \textbf{Risk} & \textbf{Likelihood} & \textbf{Impact} & \textbf{Mitigation} \\
        \midrule
        Demographic label noise, particularly UTKFace's model-estimated age labels & Medium & Medium & Report subgroup \gls{symb:n} and 95\% confidence intervals alongside every metric; cross-validate against FairFace's manually annotated labels; document the limitation explicitly \\
