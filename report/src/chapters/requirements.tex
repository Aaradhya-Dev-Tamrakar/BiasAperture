...
\begin{enumerate}
    \item \textbf{FR-001 (Data Ingestion):} The system shall load, validate, and standardise the FairFace benchmark dataset or a compatible custom dataset, checking image integrity and aligning demographic annotation fields (race or ethnicity, perceived gender, age group) into a common internal schema.
    ...
    \item \textbf{FR-005 (Explainability):} The system shall generate surrogate feature-attribution output (with spatial \gls{shap} analysis deferred) identifying which input features are most associated with a detected disparity, to support diagnosis of proxy-variable entanglement.
    ...
    \item \textbf{FR-010 (Regulatory Evidence Traceability):} For every audit run and every reported metric, the system shall produce a machine-readable evidence manifest (manifest ID, timestamp, code version, dependency lockfile, dataset ID/checksum/licence, model ID and mode, CLI config, random seeds, and computed metrics with sample sizes) and embed it in the exported report bundle as \texttt{manifest.json}.
    \item \textbf{FR-011 (Calibration and Threshold Audits):} The system shall support computing per-group calibration curves, expected calibration error (ECE), and thresholded metrics (precision@\(k\), recall at a fixed false positive rate), allowing auditors to request thresholded disparity tables and visualize calibration by subgroup.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{NFR-004 (Performance):} A full-dataset evaluation over FairFace's 97{,}698 released images (86{,}744 train + 10{,}954 validation) shall complete within 4 hours on a single mid-range \gls{gpu}; a stratified development subset of \(n = 5{,}000\) images shall complete within 30 minutes on \gls{cpu} alone.
    ...
    \item \textbf{NFR-008 (Explainability Performance):} Surrogate feature attribution (with spatial \gls{shap} analysis deferred) shall be computed only on flagged disparities rather than the full dataset by default, to keep explainability overhead a small fraction of total runtime.
    \item \textbf{NFR-009 (Reproducibility and Report Integrity):} Each report bundle shall include the code version (git commit SHA or container digest), a pinned dependency lockfile, CLI configuration, random seeds, \texttt{manifest.json}, and an integrity hash (SHA-256) for the report bundle, with optional HMAC signature support.
    \item \textbf{NFR-010 (Statistical Power Guidance):} The system shall provide an on-demand power-analysis utility estimating minimum detectable effect size given subgroup sample sizes, significance level \gls{symb:alpha}, and statistical power (\(1-\beta\)), presenting sensitivity curves for small subgroups.
\end{enumerate}
...
\begin{itemize}
    ...
    \item \textbf{Explainability:} Demographic-dummy surrogate attribution (scikit-learn; \gls{shap} deferred).
