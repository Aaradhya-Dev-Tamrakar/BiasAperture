...
\begin{enumerate}
    ...
    \item \textbf{FR-010 (Intersectional and Multiclass Cohort Decomposition):} The system shall decompose multi-class demographic attributes (such as the 7-class race and ethnicity taxonomy) and evaluate pairwise intersectional demographic cohorts (e.g., race $\times$ perceived gender) using One-vs-Rest (OvR) binarization, enabling disparity auditing across non-binary protected attributes.
    \item \textbf{FR-011 (Audit Provenance and Run Manifest):} The system shall record and embed an immutable audit manifest capturing input dataset paths, file checksums, target model identifiers, execution timestamps, random seeds, and runtime dependency environment versions, ensuring verifiable provenance for audit reproducibility.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{NFR-008 (Explainability Overhead Bounding):} Surrogate feature attribution (with spatial \gls{shap} analysis deferred) shall be computed only on flagged disparities rather than the full dataset by default, to keep explainability overhead a small fraction of total runtime.
    \item \textbf{NFR-009 (Strict Diagnostic Scope Boundary):} The system shall enforce a strict diagnostic boundary and shall not perform model retraining, weight fine-tuning, algorithmic parameter debiasing, or synthetic image generation.
    \item \textbf{NFR-010 (Offline Self-Containment and Data Privacy):} The system shall execute fully within a local offline environment without transmitting facial images or biometric predictions to external networks, and all generated compliance reports shall render without external CDN dependencies.
\end{enumerate}
