...
**Version:** 1.2.0 (Audit-Grade with Failure Branch & Multi-Class Semantics)  
...
| **R-015** | Stream B (Reporting) | The compliance report generates as a single, self-contained flat `.html` file with embedded inline CSS and base64 data-URIs, functioning completely offline with 0 external network requests. | BiasAperture WP3 Report Scaffolding Specification | **REPRODUCIBLE** *(Contract)* | Automated regex contract test fixture parses sample HTML and asserts 0 external scripts, 0 external stylesheet links, and 0 external image URLs. | `src/tests/test_offline_report_contract.py::test_offline_html_contract_validation` |
...
| **R-018** | Stream F (NIST RMF) | BiasAperture instruments the **Measure** core function of the NIST AI Risk Management Framework (NIST AI 100-1), specifically Measure 2.11, Measure 1.1, and Measure 1.3 (noting Measure 1.3 concerns evaluation participation, while dual-backends provide software cross-checking). | NIST AI Risk Management Framework 1.0 (January 2023) | **VERIFIED** | Subcategory mapping confirms $n < 30$ guard structurally satisfies Measure 1.1 (documenting unmeasurable risks). | `docs/research/HIGH_LEVEL_SYNTHESIS.md` §5 |
...
| **INV-002** | *`scipy.stats.bootstrap(method='BCa')` can be used directly for BCa confidence intervals on fairness metrics.* | Executing `scipy.stats.bootstrap` on multi-group rate differences threw `ValueError: multi-sample statistics are not supported with method='BCa'`. | Designed and implemented custom vectorized BCa bootstrap engine with jackknife delete-$d$ acceleration. |
